...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])], explored: Set[Block]): Stream[(Block, List[Move])] = {
    if (initial.isEmpty) Stream.empty
      ...
      val moves = for {
        (block, move) <- initial
        neighbors <- neighborsWithHistory(block, move)
      } yield neighbors

      initial ++ from(newNeighborsOnly(moves, explored), explored ++ moves.map(_._1))
    }
  }
  ...
  lazy val solution: List[Move] = {
    (for {
      path <- pathsToGoal
    } yield path._2.reverse).headOption.getOrElse(List())
  }
}