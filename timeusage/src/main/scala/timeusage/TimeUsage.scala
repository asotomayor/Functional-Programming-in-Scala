...
object TimeUsage extends TimeUsageInterface {
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val primaryNeedsColumns = columnNames.filter(s => s.startsWith("t01") ||
      s.startsWith("t11") || s.startsWith("t03") ||
      s.startsWith("t1801") || s.startsWith("t1803")).map(col)

    val workingActivitiesColumns = columnNames.filter(s => s.startsWith("t05") ||
      s.startsWith("t1805")).map(col)

    val otherActivitiesColumns = columnNames.filter(
      s=> s.startsWith("t02") || s.startsWith("t04") ||
