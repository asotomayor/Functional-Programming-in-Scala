...
trait DecoderInstances {
  ...
  implicit def listDecoder[A](implicit decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromFunction {
      case Json.Arr(l) => {
        val decodedElements = l.map(decoder.decode(_))
        val z: Option[List[A]] = Some(List())

        decodedElements.foldRight(z)((el, res) => res match {
          case None => None
          case Some(l) => el match {
            case None => None
  ...
  def field[A](name: String)(implicit decoder: Decoder[A]): Decoder[A] =
    Decoder.fromFunction {
      case Json.Obj(m) => m.get(name) match {
        case None => None
        case Some(v) => decoder.decode(v)
      }
    }
}
